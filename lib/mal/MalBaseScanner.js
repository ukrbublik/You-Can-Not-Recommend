...
class MalBaseScanner {
  ...
  grabByIds(opts) {
    ...
    if (cls.sharedListsOfIds === undefined)
      cls.sharedListsOfIds = {};

    //Start new iter or get already started (grabIter)
    //Promise result - boo, true if started/continued task, else false
    let startGrab = (retry = -1) => {
      ...
      return this.redis.mgetAsync([nextIndKey, grabIterKey, 
        dateIterStartKey, dateIterEndKey, retryIdsKey
      ]).then(([rNextInd, rGrabIter, rDateIterStart, rDateIterEnd, rRetryIds]) => {
        ...
        rRetryIds = rRetryIds !== null && rRetryIds.toString() !== '' 
          ? rRetryIds.toString().split(',').map(Number) : [];
        ...
        if (rNextInd === null) {
          if (rRetryIds.length) {
            ...
            let dateIterStart = new Date();
            multi.set(dateIterStartKey, dateIterStart.getTime());
            multi.del(dateIterEndKey);
            multi.hset(processingIdsKey, this.scannerId, ''); //mark of busy scanner
            return Promise.resolve().then(() => {
              ...
              return multi.execAsync().then((replies) => {
                if (replies === null) {
                  ...
                  cls.sharedListsOfIds[opts.key] = listOfIds;
          ...
          let multi = this.redis.multi();
          multi.hset(processingIdsKey, this.scannerId, ''); //mark of busy scanner
          let dateIterStart = new Date(rDateIterStart);
          return this.redis.hgetAsync(errorIdsKey, this.scannerId).then((rErrorIds) => {
            errorIds = rErrorIds ? rErrorIds.split(',').map(Number) : [];
            let getListOfIdsPromise = (cls.sharedListsOfIds[opts.key] ? Promise.resolve(true) 
              : this.redis.getAsync(new Buffer(listOfIdsKey)));
            return getListOfIdsPromise.then((rListOfIds) => {
              if (rListOfIds !== null) {
                opts.isByListOfIds = true;
                if (rListOfIds === true) {
                  //get shared
                  listOfIds = cls.sharedListsOfIds[opts.key];
                } else {
                  //copy from redis
    ...
    //Promise result - count of picked ids or -1 if already in process of picking 
    // lost ids/next portion
    let grabLostOrNext = () => {
      if (grabbingNext)
    ...
    //Promise result - [<number of repicked ids>, <are there more lost ids?>]
    let regrabLost = () => {
    ...
    //Promise result - number of picked ids 
    // (if 0 - no more portions to pick, need to complete iter)
    ...
    let checkIfShouldEnd = () => {
      if (processingIds.length == 0) {
        if (shouldStopBecauseNoData || shouldStopBecauseErrors) {
          //mark that current scanner is ready to end
          this.redis.hdel(processingIdsKey, this.scannerId);
        }
    ...
    //Promise result - bool <found lost ids?>
        ...
        if (lostScannerIds.length) {
          ...
          .then((lostScannersItemIds) => {
            ...
            for (i = 0 ; i < lostScannerIds.length ; i++) {
              ...
              if (lostItemIds !== null) {
                let itemIds = lostItemIds === '' ? [] 
                  : lostItemIds.split(',').map(Number).filter((v) => (v >= 0));
      ...
      return this.redis.hgetallAsync(errorIdsKey).then((rErrorIds) => {
        ...
        .then(([rNextInd, rGrabIter]) => {
          ...
          let allErrIds = rErrorIds === null ? null : _.uniq(Object.keys(rErrorIds)
            .map((k) => !rErrorIds[k] ? [] : rErrorIds[k].split(','))
            .reduce((prev, curr) => prev.concat(curr))
            .map(Number)
            .filter((v) => (v >= 0))
          ).sort((a, b) => (a - b));
          let isAlreadyCompleted = (rGrabIter == grabIter && rNextInd === null);
          //Rare case when scanner A already completed current iter before current 
          // and some scanner X was fast enough to already start new iter right after this
          let isAlreadyStartedNewIter = (rGrabIter != grabIter);
          if (isAlreadyStartedNewIter) {
            isAlreadyCompleted = true;
            console.log("[" + this.id + "] " + "! Some scanner already started new iter " 
              + rGrabIter);
          }
          let promise;
          if (!isAlreadyCompleted) {
            let multi = this.redis.multi();
            let dateIterEnd = new Date();
            multi.del(nextIndKey);
            multi.del(listOfIdsKey);
            delete cls.sharedListsOfIds[opts.key];
            ...
            if (replies === null) {
              ...
              if (isAlreadyStartedNewIter) {
                ...
                .then(([rRetryIds, rCntSuccessIds, rCntNotFoundIds, rNotFoundIdsAfterBiggest,
                 rBiggestFoundId]) => {
                  rRetryIds = !rRetryIds ? [] : rRetryIds.split(',');
                  rNotFoundIdsAfterBiggest = !rNotFoundIdsAfterBiggest ? [] 
                   : rNotFoundIdsAfterBiggest.split(',');
                  let cntNotFoundIdsAfterBiggest = rNotFoundIdsAfterBiggest.length;
                  ...
                  rBiggestFoundId = rBiggestFoundId === null ? 0 : parseInt(rBiggestFoundId);
                  let cntRetryIds = rRetryIds.length;
                  //if (rRetryIds)
    ...
    .then((started) => {
      if (!started)
        ...
        .then((res) => {
          res.isComplete = (!res.stopped && res.cntRetryIds == 0);
          if (res.stopped || res.cntRetryIds > 0)
            res.retry = true;
