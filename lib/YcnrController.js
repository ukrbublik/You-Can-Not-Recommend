...
class YcnrController {
  ...
  getUserByLogin(userLogin) {
    return this.db.oneOrNone("select * from malrec_users where login = $(userLogin)", {
      userLogin: userLogin
    });
  }
   ...
   */
  getUserById(userId) {
    return this.db.oneOrNone("select * from malrec_users where id = $(userId)", {
      userId: userId
    });
  }
   ...
   */
  loadItems() {
    //todo load from db   this.items = { 1: null, 2: null, .. }
  }
  ...
  recommendItemsForUser(user, limit = 20) {
    //todo: exclude unrated_items
    //todo: be careful with franchises - only 1 fr in recs, possible exclude already watched tr
    
    // todo later: Combine EMF with other approaches to get best results:
    // 0. !!! calc rmse for these combos, not only rmf
    // 1. anime-2-anime user's explicit recs (malrec_items_recs)
    // 2. kNN recs (closest users, use most_rated_items in db)
    // 3. social recs (need to grab MAL clubs, friends)

//todo ... saveCalcResults


    let err = this.emf.recommender.getCanPredictError();
    if (err)
    ...
    return Promise.all([
      ...
      let userFactors = this.emf.recommender.getFactorsRowSync('byUser', userId0);
      let minRecommendRating = this.minRecommendRating; //todo: take to account user's avg rating
      for (let itemId0 = 0 ; itemId0 < this.itemsCount ; itemId0++) { //todo instead fetch for this.items
        let itemId1 = itemId0+1;
        if (!skipItemIds[itemId]) {
          let predict = this.emf.recommender.predictSync(userId0, itemId0, userFactors);
