...
class EmfMaster extends EmfManager {
  ...
  m_fetchPortionTrainAls(w, k) {
    ...
    if (this.stepType == 'byUser')
      sql = "\
        ...
        WHERE r.dataset_type IN (1, 2) \
         AND r.user_list_id > ${rowIdFrom} AND r.user_list_id <= ${rowIdTo} \
      ...
      sql = "\
        ...
        WHERE r.dataset_type IN (1, 2) \
         AND r.item_id > ${rowIdFrom} AND r.item_id <= ${rowIdTo} \
  ...
  m_fetchPortionRmse(w, k) {
    ...
    this.db.any(sql, {
      limit: limit,
      offset: offset,
      dataset_type: this.stepType == 'rmseValidate' ? 2 : 3,
    }).then((data) => {
      let t2 = Date.now();
      this.m_processFetchedPortionRmseOrSgd(w, k, data);
    });
