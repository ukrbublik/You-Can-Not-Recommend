...
class EmfManager extends EmfBase {
  ...
  init(config, options = {}) {
    ...
    return Promise.all([
      ...
      return this.isRecommender ? Promise.resolve() : 
        this.recommender.init(config, options);
    }).then(() => {
    ...
    return new Promise((resolve, reject) => {
      if (canReuse) {
        ...
        if (this.options.keepFactorsOpened) {
          ...
          Promise.all([this._loadFactorsFromFiles(), this._loadBiasesFromFiles()]).then(() => {
            resolve(true);
          });
        } else {
          resolve(true);
   ...
   * Params in resolve: recreated, extended
   */
  ...
  saveCalcResults(calcInfo) {
    //todo_later: (?) if there was train for only new ratings, save only factors of affected U/I
    if (this.isRecommender)
      throw new Error("Only for trainer");
    ...
    return new Promise((resolve, reject) => {
      let savePromise = new Promise((resolve, reject) => {
        ...
        //for lowmem factors are already live saved to files, so do nothing here for factors
        Promise.all([saveBiasesPromise]).then(() => {
          //save info
          if(this.options.lowmem)
      ...
      savePromise.then(() => {
        ...
        this.recommender._saveCalcResultsToRecommender(calcInfo, readStreams).then(() => {
          if (!this.options.keepFactorsOpened) {
            this.detachSharedFactors();
          }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (readStreams === null && !lowmemUseTemp) { //trainer has lowmem==1
          ...
          Promise.all([this._loadFactorsFromFiles(), this._loadBiasesFromFiles()]).then(() => {
            ...
            this.copyCalcResults(this.options.factorsReadyPath, 
              this.options.factorsWorkingPath).then(() => {
                resolve();
              });
          });
      } else {
