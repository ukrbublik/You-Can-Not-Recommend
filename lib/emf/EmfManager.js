...
class EmfManager extends EmfBase {
  ...
  loadCalcResults() {
    ...
    if (!canReuse) {
      if (!this.isRecommender && this.options.lowmem) {
        //this.factorPath == this.factorsWorkingPath
        //this.recommender.factorPath == this.factorsReadyPath
        if (!fs.existsSync(this.factorsWorkingPath))
          fs.mkdirSync(this.factorsWorkingPath);
        if (this._canReuseCalcResults(this.recommender.lastCalcInfo)) {
          console.log(this.constructor.name + ": " 
            + "Copying ready calc result to working...");
          return this.copyCalcResults(this.factorsReadyPath, 
            this.factorsWorkingPath)
          .then(() => {
            return this.loadCalcResults();
          });
        }
    ...
    if (canReuse) {
      ...
      if (calcInfo) {
        if (this.isRecommender) {
          //this.factorPath == this.factorsReadyPath
          console.log(this.constructor.name + ": " 
  ...
  _loadSharedFactorsForTrain() {
    ...
    /*
    //todo: зачем была эта ф-ия, удалить?
    let beforeReturn = () => {
      if(this.options.lowmem) {
  ...
  _saveCalcResultsToRecommender(calcInfo, readStreams) {
    ...
    if (readStreams === null && !lowmemUseTemp) { //trainer has lowmem==1
      ...
      this.deleteCalcResultsSync(this.factorsReadyPath);
      if (fs.existsSync(this.factorsReadyPath))
        fs.rmdirSync(this.factorsReadyPath);        
      fs.renameSync(this.factorsWorkingPath, this.factorsReadyPath);
      ...
      if (!fs.existsSync(this.factorsTempPath))
        fs.mkdirSync(this.factorsTempPath);
      let copyTempPromise;
      if (readStreams === null) { //trainer has lowmem==1
        //copy /factors_working to /factors_temp
        copyTempPromise = this.copyCalcResults(this.factorsWorkingPath, 
          this.factorsTempPath);
      } else {
        let saveTempPromises = [];
        for (let filename in readStreams) {
          let filePath = this.factorsTempPath + '/' + filename;
        ...
        copyTempPromise = Promise.all(saveTempPromises).then(() => {
          let calcInfoPath = this.factorsTempPath + '/' + this.calcInfoFilename;
      ...
      return copyTempPromise.then(() => {
        ...
        if (fs.existsSync(this.factorsReadyPath))
          fs.rmdirSync(this.factorsReadyPath);        
        fs.renameSync(this.factorsTempPath, this.factorsReadyPath);
