...
forceResplit=true; //todo!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
    ...
    return promise.then(() => {
      ...
      let total = Object.keys(splitCnts).map((typeKey) => splitCnts[typeKey])
      ...
      if (!isOkSplit) {
        ...
        console.log("Data are splitted to sets train/validate/test %", newPcts, 
          (!doSplit ? "" : "- complete in " + ((t2-t1)/1000).toFixed(1) + " s"),
        ...
        ").then((avgRat) => {
          ...
          return this.doUpdateStats().then(() => {
            let t4 = Date.now();
    ...
    */
      ...
      ").then((row) => {
        ...
        return Helpers.promiseWhile(() => (gotIds < maxId), () => {
          return this.db.manyOrNone("\
            select u.list_id, u.ratings_count, u.avg_rating, \
             count(r.rating) as new_ratings_count, avg(r.rating) as new_avg_rating \
            from malrec_users as u \
            inner join malrec_ratings as r \
             on r.user_list_id = u.list_id \
            where u.list_id > $(fromId) and u.list_id <= $(toId) \
             and u.is_used_for_train = true and r.dataset_type IN (1, 2) \
            group by u.list_id \
          ", {
            fromId: gotIds,
            ...
            if (rows)
              for (let row of rows) {
                row.new_ratings_count = parseInt(row.new_ratings_count);
                row.new_avg_rating = parseInt(row.new_avg_rating);
                if (row.new_ratings_count != row.ratings_count 
                 || row.new_avg_rating != row.avg_rating) {
                  updatedStats[row.list_id] = {};
                  if (row.new_ratings_count != row.ratings_count)
                    updatedStats[row.list_id]['ratings_count'] = row.new_ratings_count;
                  if (row.new_avg_rating != row.avg_rating)
    ...
    let updateItemsStats = () => {
      ...
      ").then((row) => {
        let maxId = row.max;
        let idsPerPortion = 200;
        this.workProgress = new ProgressBar('For items' 
          + ' [:bar] :current/:total :elapsed/:etas', { total: Math.ceil(maxId / idsPerPortion) });
        let gotIds = 0, processedIds = 0;
        return Helpers.promiseWhile(() => (gotIds < maxId), () => {
          return this.db.manyOrNone("\
            select i.id, i.ratings_count, i.avg_rating, \
             count(r.rating) as new_ratings_count, avg(r.rating) as new_avg_rating \
            from malrec_items as i \
            inner join malrec_ratings as r \
             on r.item_id = i.id \
            where i.id > $(fromId) and i.id <= $(toId) \
             and i.is_used_for_train = true and r.dataset_type IN (1, 2) \
            group by i.id \
          ", {
            fromId: gotIds,
            toId: (gotIds + idsPerPortion),
  ...
  doSplitToSets(splitAll, pcts) {
    ...
    return this.db.func(func, [ pcts[0], pcts[1], pcts[2] ]);
    */
