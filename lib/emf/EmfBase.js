...
const TcpSocket = require('../../quick-tcp-socket').TcpSocket; //todo npm
...
class EmfBase extends EventEmitter {
  ...
  static get DefaultOptions() {
    return {
      ...
      factorsWorkingPath: __dirname + '/../data/factors_working',
      factorsReadyPath: __dirname + '/../data/factors_ready',
      factorsTempPath: __dirname + '/../data/factors_tmp',
    };
  };
  ...
  get factorsPath() {
    return this.isRecommender || !this.options.lowmem ? 
      this.options.factorsReadyPath : this.options.factorsWorkingPath;
  }
  /**
   * Statuses:
   * <empty>
  ...
  get minRecommendRating() { return this.options.minRecommendRating[this.options.dbType] }
  ...
  getUserByLogin(userLogin) {
    return this.db.one("select * from malrec_users where login = $(userLogin)", {
      userLogin: userLogin
    });
  }
   ...
   */
  getUserById(userId) {
    return this.db.one("select * from malrec_users where id = $(userId)", {
      userId: userId
    });
  }
